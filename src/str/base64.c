...
#if 0
sg_vsstr *sg_base64_easy_enc_buf(void *data, size_t size)
{
    ...
    b64_str = sg_vsstrndup(b64_buf, b64_size - 1);
    sg_log_inf("b64_str len %lu.", (size_t)sg_vsstrlen(b64_str));
    free(b64_buf);
    return b64_str;
}

struct sg_vsbuf *sg_base64_easy_dec_buf(const char *base64_str)
{
    ...
    dec_size = cnt;
    sg_vsbuf_insert(buf, dec_buf, dec_size);
...
//struct sg_vsbuf *sg_base64_dec(const char *b64_str, size_t b64_str_len)
{
